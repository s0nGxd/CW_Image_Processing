...
def component_touching_seed(mask, seed_mask):
    """
    Keep only the connected component in mask that overlaps the seed mask.
    If nothing overlaps, fall back to the largest component.
    """
    num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(mask, connectivity=8)
    if num_labels <= 1:
        return mask
...
def fill_holes(mask):
    """
    Fill holes inside a binary object mask.
    """
    flood = mask.copy()
    h, w = mask.shape
...
def safe_grabcut(img, init_mask, iters=5):
    """
    Run GrabCut safely. If initialization is invalid, return the
    probable/sure foreground from the init mask instead of crashing.
    """
    gc_mask = init_mask.copy()

    # Make sure there is at least some sure foreground
    ...
    # Make sure there is at least some sure background
...
def get_nucleus_mask(img):
    """
    Detect nucleus using an adaptive stain score instead of only fixed thresholds.
    More stable across different lighting/staining conditions.
    """
    ...
    # Plausible nucleus color range
    color_mask1 = cv2.inRange(hsv, (105, 30, 20), (175, 255, 210))
    color_mask2 = cv2.inRange(a, 128, 210)
    color_mask3 = cv2.inRange(b, 70, 150)
...
def expand_box(x, y, w, h, img_w, img_h, scale=4.0, min_size=150):
    """
    Expand a box around the nucleus so the full WBC is likely inside the crop.
    """
    cx = x + w / 2
    cy = y + h / 2
...
def estimate_non_background(crop):
    """
    Estimate pixels that are not plain background by comparing each pixel
    to border statistics in LAB color space.
    """
    ...
    z = np.sqrt((((lab.astype(np.float32) - mean) / std) ** 2).sum(axis=2))
    ...
    non_bg = cv2.morphologyEx(non_bg, cv2.MORPH_CLOSE, k)

    return non_bg, border_mask, lab, hsv
...
def segment_wbc(image_path, save_mask_path=None, save_output_path=None, debug_dir=None):
    ...
    x1, y1, x2, y2 = expand_box(x, y, w, h, W, H, scale=4.0, min_size=150)

    crop = original[y1:y2, x1:x2].copy()
    nucleus_crop = nucleus[y1:y2, x1:x2]

    non_bg, border_mask, lab_crop, hsv_crop = estimate_non_background(crop)
    l_crop = lab_crop[:, :, 0]
    s_crop = hsv_crop[:, :, 1]

    # Grow a seed around nucleus
    seed = cv2.dilate(
        nucleus_crop,
    ...
    # Keep only non-background connected to nucleus
    candidate = component_touching_seed(non_bg, seed)
    ...
    # probable foreground = candidate
    gc_mask[candidate > 0] = cv2.GC_PR_FGD

    # sure foreground = nucleus
    gc_mask[nucleus_crop > 0] = cv2.GC_FGD

    # sure background = border + very bright low-sat areas
    sure_bg = (
        (border_mask > 0) |
        ((l_crop > 242) & (s_crop < 25))
    )
    ...
    # Cleanup
    mask_crop = component_touching_seed(mask_crop, seed)
    ...
    # Put crop result back into full-size image
    ...
    if debug_dir is not None:
        ...
        candidate_full = np.zeros((H, W), np.uint8)
        candidate_full[y1:y2, x1:x2] = candidate
        ...
        cv2.imwrite(os.path.join(debug_dir, "04_non_background_candidate.png"), candidate_full)
        cv2.imwrite(os.path.join(debug_dir, "05_final_mask.png"), mask)
        cv2.imwrite(os.path.join(debug_dir, "06_output.png"), output)
